...
static constexpr bool INDEX_OFFSET = true;
...
class RectangularMatrix {
  protected:
    ...
    void destroy() noexcept {
      if(m_entries) {
        for(size_t i = 0; i < order(); i++) m_entries[i].~T();
        delete[] m_entries;
        m_entries = nullptr;
      }
    }
  ...
  public:
    ...
    T& at(size_t row, size_t column) {
      if(INDEX_OFFSET && row * column == 0) throw std::runtime_error("Index of matrices start at 1,1. Please start with 1, not 0 (or change the offset to false in header file).");
      if(row > m_rows + INDEX_OFFSET || column > m_columns + INDEX_OFFSET) throw std::runtime_error("Index used to access the elements can't be greater than the row/column present.");
      return *(m_entries + m_columns * (row - INDEX_OFFSET) + (column - INDEX_OFFSET));
    }
