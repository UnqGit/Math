...
_MTEMPL_ requires _NOTHR_DSTR_ class _NODISC_ Matrix {
    ...
    public:
        ...
        Matrix(const size_t size) : m_order(_MATRIX_ Order(size, size)) {
            ...
            if (zero_exists)
                for (size_t i = 0; i < size; i++) {
                    _MEM_ allocate_mem_2d_safe_continuous<T>(m_data, i, size);
                    _MEM_ mem_2d_safe_uninit_fill_n_continuous<T>(m_data[i], _GET_ZERO_, size, m_data, i, size);
                }
            else if constexpr (_DFLT_CSTR_)
                for (size_t i = 0; i < size; i++) {
                    _MEM_ allocate_mem_2d_safe_continuous<T>(m_data, i, size);
                    _MEM_ mem_2d_safe_uninit_valcon_n_continuous<T>(m_data[i], size, m_data, i, size);
                }
        }
        ...
        requires _CPY_CSTR_ : m_order(_MATRIX_ Order(size, size)) {
            _ORD_ZERO_RET_ switch(construct_rule) {
                ...
                case _MATRIX_ CSR::lower_half :
                    *this = Matrix(size, secondary_value, primary_value, _MATRIX_ CSR::upper_half); return;
                case _MATRIX_ CSR::bottom_right_triangle :
                    *this = Matrix(size, secondary_value, primary_value, _MATRIX_ CSR::top_left_triangle); return;
                case _MATRIX_ CSR::bottom_left_triangle :
                    *this = Matrix(size, secondary_value, primary_value, _MATRIX_ CSR::top_right_triangle); return;
                default: break;
            }
            m_data = _MEM_ allocate_2d_safe_memory<T>(size, size);
            T *cache_data;
            bool is_primary = 0;
            switch(construct_rule) {
                case _MATRIX_ CSR::full :
                    for (size_t i = 0; i < size; i++) 
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], primary_value, size, m_data, i, size, size);
                    return;
                case _MATRIX_ CSR::upper_half :
                    for (size_t i = 0; i < (size>>1); i++) 
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], primary_value, size, m_data, i, size, size);
                    for (size_t i = (size>>1); i < size; i++)
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], secondary_value, size, m_data, i, size, size);
                    return;
                case _MATRIX_ CSR::left_half :
                    for (size_t i = 0; i < size; i++) {
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], primary_value, (size>>1), m_data, i, size, size);
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i] + (size>>1), secondary_value, (size>>1) + (size & 1), m_data, i, size, size);
                    }
                    return;
                case _MATRIX_ CSR::top_left_triangle :
                    for (size_t i = 0; i < size; i++) {
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], primary_value, size - i, m_data, i, size, size);
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i] + size - i, secondary_value, i, m_data, i, size, size);
                    }
                    return;
                case _MATRIX_ CSR::top_right_triangle :
                    for (size_t i = 0; i < size; i++) {
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], secondary_value, i, m_data, i, size, size);
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i] + i, primary_value, size - i, m_data, i, size, size);
                    }
                    return;
                case _MATRIX_ CSR::main_diagonal :
                    for (size_t i = 0; i < size; i++) {
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], secondary_value, i, m_data, i, size, size);
                        _MEM_ mem_2d_safe_construct_at<T>(m_data[i] + i, m_data, i, size, size, primary_value);
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i] + i + 1, secondary_value, size - i - 1, m_data, i, size, size);
                    }
                    return;
                case _MATRIX_ CSR::off_diagonal :
                    for (size_t i = 0; i < size; i++) {
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], secondary_value, size - 1 - i, m_data, i, size, size);
                        _MEM_ mem_2d_safe_construct_at<T>(m_data[i] + size - 1 - i, m_data, i, size, size, primary_value);
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i] + size - i, secondary_value, size - i, m_data, i, size, size);
                    }
                    return;
                case _MATRIX_ CSR::top_left_quarter :
                    for (size_t i = 0; i < (size>>1); i++) {
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], primary_value, (size>>1), m_data, i, size, size);
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i] + (size>>1), secondary_value, ((size>>1) + (size&1)), m_data, i, size, size);
                    }
                    for (size_t i = (size>>1); i < size; i++)
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], secondary_value, size, m_data, i, size, size);
                    return;
                case _MATRIX_ CSR::top_right_quarter :
                    for (size_t i = 0; i < (size>>1); i++) {
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], secondary_value, (size>>1), m_data, i, size, size);
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i] + (size>>1), primary_value, ((size>>1) + (size&1)), m_data, i, size, size);
                    }
                    for (size_t i = (size>>1); i < size; i++)
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], secondary_value, size, m_data, i, size, size);
                    return;
                case _MATRIX_ CSR::bottom_left_quarter :
                    for (size_t i = 0; i < (size>>1); i++)
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], secondary_value, size, m_data, i, size, size);
                    for (size_t i = (size>>1); i < size; i++) {
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], primary_value, (size>>1), m_data, i, size, size);
                    ...
                    return;
                case _MATRIX_ CSR::bottom_right_quarter :
                    for (size_t i = 0; i < (size>>1); i++)
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], secondary_value, size, m_data, i, size, size);
                    for (size_t i = (size>>1); i < size; i++) {
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], secondary_value, (size>>1), m_data, i, size, size);
                    ...
                    return;
                case _MATRIX_ CSR::alternate :
                    for (size_t i = 0; i < size; i++) {
                        is_primary = (i % 2 == 0);
                        cache_data = m_data[i];
                        for (size_t j = 0; j < size; j++) {
                            _MEM_ mem_2d_safe_construct_at<T>(cache_data + j, m_data, i, size, size, is_primary ? primary_value : secondary_value);
                            is_primary = !is_primary;
                        }
                    }
                    return;
                case _MATRIX_ CSR::alternate_row :
                    for (size_t i = 0; i < size; i++) {
                        is_primary = (i % 2 == 0);
                        _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], is_primary ? primary_value : secondary_value, size, m_data, i, size, size);
                    }
                    return;
                case _MATRIX_ CSR::alternate_column :
                    for (size_t i = 0; i < size; i++) {
                        cache_data = m_data[i];
                        is_primary = true;
                        for (size_t j = 0; j < size; j++) {
        ...
        Matrix(const size_t size, const T &primary_value, const _MATRIX_ ConstructSquareRule construct_rule) 
        requires _CPY_CSTR_ {
            if (size == 0) return;
            _ZERO_EXISTS_
            _NO_ZERO_COND_ throw _STD_ logic_error("Cannot construct the Matrix for this type because neither zero value is stored and neither is it default constructible.");
            if (zero_exists) *this = Matrix(size, primary_value, _GET_ZERO_, construct_rule);
            else if constexpr (_DFLT_CSTR_) *this = Matrix(size, primary_value, T{}, construct_rule);
        }
    ...
    public:
        Matrix(const _MATRIX_ Order &order, const _MATRIX_ ConstructAllocateRule construct_rule = _MATRIX_ CAR::zero) : m_order(order) {
            ...
            if (construct_rule == _MATRIX_ CAR::possible_garbage) {
                ...
                m_data = _MEM_ allocate_2d_safe_memory<T>(row, col);
                if constexpr (!_STD_ is_trivially_constructible_v<T>)
                    for (size_t i = 0; i < row; i++) {
                        if constexpr (!_DFLT_CSTR_) _MEM_ mem_2d_safe_uninit_fill_n<T>(m_data[i], _GET_ZERO_, col, m_data, i, row, col);
                        else _MEM_ mem_2d_safe_uninit_valcon_n<T>(m_data[i], col, m_data, i, row, col);
                    }
            }
            else {
                if (!zero_exists)
                ...
                for (size_t i = 0; i < row; i++) {
                    ...
                    _MEM_ mem_2d_safe_uninit_fill_n_continuous<T>(m_data[i], _GET_ZERO_, col, m_data, i, col);
                }
            }
        }
        Matrix(const size_t row, const size_t column, const _MATRIX_ ConstructAllocateRule construct_rule = _MATRIX_ CAR::zero) : Matrix(_MATRIX_ Order(row, column), construct_rule) {}
    ...
    public:
        ...
        requires _CPY_CSTR_ {
            ...
            switch (construct_rule) {
                case _MATRIX_ COR::horizontal :
                    ...
                    m_data = _MEM_ allocate_memory<T*>(1);
                    _MEM_ allocate_mem_2d_safe_continuous<T>(m_data, 0, size);
                    _MEM_ mem_2d_safe_uninit_copy_n_continuous<T>(m_data[0], size, data, m_data, 0, size);
                    return;
                case _MATRIX_ COR::vertical :
                    m_order = _MATRIX_ Order(size, 1);
                    m_data = _MEM_ allocate_memory<T*>(size);
                    for (size_t i = 0; i < size; i++) {
                        _MEM_ allocate_mem_2d_safe_continuous<T>(m_data, i, 1);
                        _MEM_ mem_2d_safe_construct_at_continuous<T>(m_data[i], m_data, i, size, data[i]);
                    }
                    return;
                case _MATRIX_ COR::main_diagonal :
                    m_data = _MEM_ allocate_memory<T*>(size);
                    m_order = _MATRIX_ Order(size, size);
                    for (size_t i = 0; i < size; i++) {
                        _MEM_ allocate_mem_2d_safe_continuous<T>(m_data, i, size);
                        _MEM_ mem_2d_safe_uninit_fill_n_continuous<T>(m_data[i], fallback_val, i, m_data, i, size);
                        _MEM_ mem_2d_safe_construct_at_continuous<T>(m_data[i] + i, m_data, i, size, data[i]);
                        _MEM_ mem_2d_safe_uninit_fill_n_continuous<T>(m_data[i] + i + 1, fallback_val, size - i - 1, m_data, i, size);
                    }
                    return;
                case _MATRIX_ COR::off_diagonal :
                    m_data = _MEM_ allocate_memory<T*>(size);
                    m_order = _MATRIX_ Order(size, size);
                    for (size_t i = 0; i < size; i++) {
                        _MEM_ allocate_mem_2d_safe_continuous<T>(m_data, i, size);
                        _MEM_ mem_2d_safe_uninit_fill_n_continuous<T>(m_data[i], fallback_val, size - i - 1, m_data, i, size);
                        _MEM_ mem_2d_safe_construct_at_continuous<T>(m_data[i] + size - i - 1, m_data, i, size, data[i]);
                        _MEM_ mem_2d_safe_uninit_fill_n_continuous<T>(m_data[i] + size - i, fallback_val, i, m_data, i, size);
                    }
                    return;
            }
        }
        ...
        requires _CPY_CSTR_ {
            ...
            switch (construct_rule) {
                ...
                case _MATRIX_ COR::off_diagonal :
                    _NO_ZERO_COND_ throw _STD_ logic_error("Cannot construct the Matrix for this type because neither zero value is stored and neither is it default constructible.");
                    if constexpr (!_DFLT_CSTR_) *this = Matrix(data, size, construct_rule, _GET_ZERO_);
        ...
        requires _CPY_CSTR_ : m_order(order) {
            ...
            for (size_t i = 0; i < row; i++) {
                _MEM_ allocate_mem_2d_safe_continuous<T>(m_data, i, col);
                _MEM_ mem_2d_safe_uninit_copy_n_continuous<T>(m_data[i], col, data + i * col, m_data, i, col);
            }
        }
    ...
    public:
        ...
        requires _CPY_CSTR_ : m_order(order) {
            ...
            for (size_t i = 0; i < row; i++) {
                ...
                switch(is_remaining) {
                    case true:
                        _MEM_ mem_2d_safe_uninit_copy_n_continuous<T>(m_data[i], _STD_ min(size - constructed_items, col), data + constructed_items, m_data, i, col);
                        constructed_items += _STD_ min(size - constructed_items, col);
                        if (constructed_items % col == 0) break;
                        [[fallthrough]];
                    case false:
                        if (constructed_items % col)
                            _MEM_ mem_2d_safe_uninit_fill_n_continuous<T>(m_data[i] + (constructed_items % col), fallback_val, constructed_items % col, m_data, i, col);
                        else _MEM_ mem_2d_safe_uninit_fill_n_continuous<T>(m_data[i], fallback_val, col, m_data, i, col);
    ...
    public:
        ...
        requires _CPY_CSTR_ {
            ...
            switch (construct_rule) {
                case _MATRIX_ COR::horizontal :
                    ...
                    m_data = _MEM_ allocate_memory<T*>(1);
                    _MEM_ allocate_mem_2d_safe_continuous<T>(m_data, 0, size);
                    _MEM_ mem_2d_safe_uninit_copy_continuous<T>(m_data[0], arr.begin(), arr.end(), m_data, 0, size);
                    return;
                case _MATRIX_ COR::vertical :
                    m_order = _MATRIX_ Order(size, 1);
                    m_data = _MEM_ allocate_memory<T*>(size);
                    for (size_t i = 0; i < size; i++) {
                        _MEM_ allocate_mem_2d_safe_continuous<T>(m_data, i, 1);
                        _MEM_ mem_2d_safe_uninit_copy_n_continuous<T>(m_data[i], 1, Iter, m_data, i, 1); // This instead of _MEM_ mem_2d_safe_construct_at_continuous because we would have to pass *Iter as an argument but if the dereferencing throws an exception, it would leak memory.
                    }
                    return;
                case _MATRIX_ COR::main_diagonal :
                    m_order = _MATRIX_ Order(size, size);
                    ...
                    for (size_t i = 0; i < size; i++) {
                        _MEM_ allocate_mem_2d_safe_continuous<T>(m_data, i, size);
                        _MEM_ mem_2d_safe_uninit_fill_n_continuous<T>(m_data[i], fallback_val, i, m_data, i, size);
                        _MEM_ mem_2d_safe_uninit_copy_n_continuous<T>(m_data[i] + i, 1, Iter, m_data, i, size);
                        _MEM_ mem_2d_safe_uninit_fill_n_continuous<T>(m_data[i] + i + 1, fallback_val, size - i - 1, m_data, i, size);
                        if constexpr ( !noexcept( ++_STD_ declval<_STD_ decay_t<decltype(Iter)>>() )) try {++Iter;} _CATCH_MEM_ERR_CONT_(m_data, i + 1, size)
                        else ++Iter;
                    }
                    return;
                case _MATRIX_ COR::off_diagonal :
                    ...
                    for (size_t i = 0; i < size; i++) {
                        _MEM_ allocate_mem_2d_safe_continuous<T>(m_data, i, size);
                        _MEM_ mem_2d_safe_uninit_fill_n_continuous<T>(m_data[i], fallback_val, size - i - 1, m_data, i, size);
                        _MEM_ mem_2d_safe_uninit_copy_n_continuous<T>(m_data[i] + size - 1 - i, 1, Iter, m_data, i, size);
                        _MEM_ mem_2d_safe_uninit_fill_n_continuous<T>(m_data[i] + size - i, fallback_val, i, m_data, i, size);
                        if constexpr ( !noexcept( ++_STD_ declval<_STD_ decay_t<decltype(Iter)>>() )) try {++Iter;} _CATCH_MEM_ERR_CONT_(m_data, i + 1, size)
                        else ++Iter;
                    }
        ...
        requires _CPY_CSTR_ {
            ...
            switch (construct_rule) {
                ...
                case _MATRIX_ COR::off_diagonal :
                    _NO_ZERO_COND_ throw _STD_ logic_error("Cannot construct the Matrix for this type because neither zero value is stored and neither is it default constructible.");
                    if constexpr (!_DFLT_CSTR_) *this = Matrix(arr, construct_rule, _GET_ZERO_);
        ...
        requires _CPY_CSTR_ : m_order(order) {
            ...
            for (size_t i = 0; i < row; i++) {
                ...
                switch (is_remaining) {
                    case true :
                        j = _STD_ min(size - constructed_items, col);
                        _MEM_ mem_2d_safe_uninit_copy_n_continuous<T>(m_data[i], j, Iter, m_data, i, col);
                        constructed_items += j;
                        if (constructed_items % col == 0) break;
                        [[fallthrough]];
                    case false :
                        if (constructed_items % col == 0) _MEM_ mem_2d_safe_uninit_fill_n_continuous<T>(m_data[i], fallback_val, col, m_data, i, col);
                        else _MEM_ mem_2d_safe_uninit_fill_n_continuous<T>(m_data[i] + j, fallback_val, col - j, m_data, i, col);
                }
            }
        }
        ...
        requires _CPY_CSTR_ : m_order(order) {
            ...
            if (size < order.size()) {
                _NO_ZERO_COND_ throw _STD_ logic_error("Cannot construct the Matrix for this type because neither zero value is stored and neither is it default constructible.");
                if constexpr (!_DFLT_CSTR_) *this = Matrix(arr, order, _GET_ZERO_);
            ...
            for (size_t i = 0; i < row; i++) {
                _MEM_ allocate_mem_2d_safe_continuous<T>(m_data, i, col);
                _MEM_ mem_2d_safe_uninit_copy_n_continuous<T>(m_data[i], col, Iter, m_data, i, col);
            }
        }
    ...
    public:
        ...
        requires _CPY_CSTR_ {
            ...
            if (size == 0) return;
            if (size == 1) {
                *this = Matrix(*(arr.begin()), _MATRIX_ COR::horizontal);
                return;
            }
            size_t row_size;
            auto Iter = arr.begin();
            ...
            switch (construct_rule) {
                case _MATRIX_ CCR::shrink :
                    row_size = (*Iter).size();
                    while (Iter != end) {
                        row_size = _STD_ min(row_size, (*Iter).size());
                    ...
                    Iter = arr.begin();
                    for (size_t i = 0; i < size; i++) {
                        _MEM_ allocate_mem_2d_safe_continuous<T>(m_data, i, row_size);
                        _MEM_ mem_2d_safe_uninit_copy_n_continuous<T>(m_data[i], row_size, Iter, m_data, i, row_size);
                    }
                    return;
                case _MATRIX_ CCR::must_be_same :
                    row_size = (*Iter).size();
                    while (Iter != end) {
                        if (row_size != (*Iter).size()) throw _STD_ invalid_argument("Rows of the provided container are not of the same size and rule chosen is _MATRIX_ ConstructContainerRule::must_be_same.");
                        ++Iter;
                    }
                    m_order = _MATRIX_ Order(size, row_size);
                    if (row_size == 0) return;
                    ...
                    return;
                case _MATRIX_ CCR::expand :
                    row_size = (*Iter).size();
                    bool are_all_same = true;
                    size_t temp;
                    while (Iter != end) {
                        temp = _STD_ max(row_size, (*Iter).size());
                    ...
                    for (size_t i = 0; i < size; i++) {
                        _MEM_ allocate_mem_2d_safe_continuous<T>(m_data, i, row_size);
                        row_objects_created = _MEM_ mem_2d_safe_uninit_copy_continuous<T>(m_data[i], (*Iter).begin(), (*Iter).end(), m_data, i, row_size);
                        if (row_objects_created != row_size) {
                            if constexpr (!_DFLT_CSTR_) 
                                _MEM_ mem_2d_safe_uninit_fill_n_continuous<T>(m_data[i] + row_objects_created, _GET_ZERO_, row_size - row_objects_created, m_data, i, row_size);
                            else _MEM_ mem_2d_safe_uninit_valcon_n_continuous<T>(m_data[i] + row_objects_created, row_size - row_objects_created, m_data, i, row_size);
                        }
                        ++Iter;
                    }
                    return;
                case _MATRIX_ CCR::are_same :
                    row_size = (*Iter).size();
                    if (row_size == 0) return;
                    m_order = _MATRIX_ Order(size, row_size);
                    m_data = _MEM_ allocate_memory<T*>(size);
                    for (size_t i = 0; i < size; i++) {
                        if ((*Iter).size() != row_size) {
                            _MEM_ destroy_data_mem_err_continuous<T>(m_data, i, row_size);
                            throw _STD_ logic_error("Promised attribute _MATRIX_ ConstructConainerRule::are_same was not satisfied in construction of the Matrix.");
                        }
                        m_data[i] = _MEM_ allocate_mem_2d_safe_continuous<T>(m_data, i, row_size);
                        _MEM_ mem_2d_safe_uninit_copy_n_continuous<T>(m_data[i], row_size, Iter, m_data, i, row_size);
                        ++Iter;
                    }
                    return;
    ...
    public:
        ...
        requires (_CPY_CSTR_ || _MV_CSTR_) : m_order(order) {
            ...
            for (size_t i = 0; i < row; i++) {
                _MEM_ allocate_mem_2d_safe_continuous<T>(m_data, i, col);
                if constexpr ( noexcept(t_creation) ) { for (j = 0; j < col; j++) _STD_ construct_at(m_data[i] + j, t_creation()); }
                else _TRY_CONSTRUCT_AT_LOOP_(j, (j < col), (j++), m_data[i], t_creation()) _CATCH_DES_DATA_CONT_(m_data, i, j, col)
            }
        }
        
        Matrix(const size_t row, const size_t column, const _STD_ function<T()> &t_creation)
        requires (_CPY_CSTR_ || _MV_CSTR_) : Matrix(_MATRIX_ Order(row, column), t_creation) {}
        ...
        requires (_CPY_CSTR_ || _MV_CSTR_) : m_order(order) {
            ...
            for (size_t i = 0; i < row; i++) {
                ...
                if constexpr ( noexcept(t_creation) ) { for (j = 0; j < col; j++) _STD_ construct_at(m_data[i] + j, t_creation(i, j)); }
                else _TRY_CONSTRUCT_AT_LOOP_(j, (j < col), (j++), m_data[i], t_creation(i, j)) _CATCH_DES_DATA_CONT_(m_data, i, j, col)
            }
        }
        
        Matrix(const size_t row, const size_t column, const _STD_ function<T(size_t, size_t)> &t_creation)
        requires (_CPY_CSTR_ || _MV_CSTR_) : Matrix(_MATRIX_ Order(row, column), t_creation) {}

    public:
        Matrix(const Matrix &other) : Matrix(other.m_data, other.m_order) {} // Delegate to Matrix(T** data, const _MATRIX_ Order &order) {} constructor.
    ...
    public:
        ~Matrix() noexcept {
            if (m_data != nullptr) _MEM_ destroy_data_mem_err_continuous<T>(m_data, m_order.row(), m_order.column());
        }
        void reset() noexcept {
    ...
    public:
        ...
        _NODISC_ T &at(const size_t row, const size_t column) {
            if ((row >= m_order.row()) || (column >= m_order.column())) throw _STD_ out_of_range("Provided index does not exist within the bounds of this Matrix.");
            return m_data[row][column];
        }
    ...
    public:
        _NODISC_ _MATRIX_ Order order() const noexcept {
            return m_order;
        }
    ...
    public:
        ...
        _MATRIX_ Row<T> row(const size_t row) const {
            if (row >= m_order.row()) throw _STD_ out_of_range("Cannot provide row object for the provided row number.");
            return _MATRIX_ Row<T>(m_data[row], m_order.column());
        }
        _MATRIX_ Column<T> column(const size_t col) const {
            if (col >= m_order.column()) throw _STD_ out_of_range("Cannot access the column on the provided index as it exceeds the number of columns present in the matrix.");
            return _MATRIX_ Column<T>(m_data, col, m_order.row());
        }
        _MATRIX_ Row<T> row(const size_t row) {
            if (row >= m_order.row()) throw _STD_ out_of_range("Cannot provide row object for the provided row number.");
            return _MATRIX_ Row<T>(m_data[row], m_order.column());
        }
        _MATRIX_ Column<T> column(const size_t col) {
            if (col >= m_order.column()) throw _STD_ out_of_range("Cannot access the column on the provided index as it exceeds the number of columns present in the matrix.");
            return _MATRIX_ Column<T>(m_data, col, m_order.row());
        }
    ...
    public:
        ...
        requires compoundAddition<T> {
            ...
            if constexpr ( noexcept( _STD_ declval<_STD_ decay_t<T>>() += _STD_ declval<_STD_ decay_t<T>>() ) ) {
                ...
                T **result;
                try { result = _MEM_ allocate_2d_safe_memory<T>(row, col); }
                catch(...) { throw _STD_ runtime_error("Could not do addition for this matrix aa an error occured during memory allocation(which was required as the operator(+=) isn't noexcept)."); }
                if constexpr ( noexcept( _STD_ declval<_STD_ decay_t<T>>() + _STD_ declval<_STD_ decay_t<T>>() ) && _STD_ is_nothrow_copy_constructible_v<T> ) {
                    #pragma omp parallel for collapse(2) schedule(static)
                    for (size_t i = 0; i < row; i++)
                        for (size_t j = 0; j < col; j++)
                            _STD_ construct_at(result[i] + j, m_data[i][j] + other.m_data[i][j]);
                }
                else {
                    size_t j;
                    for (size_t i = 0; i < row; i++) {
        ...
        requires compoundSubtraction<T> {
            ...
            if constexpr ( noexcept( _STD_ declval<_STD_ decay_t<T>>() -= _STD_ declval<_STD_ decay_t<T>>() ) ) {
                ...
                T **result;
                try { result = _MEM_ allocate_2d_safe_memory<T>(row, col); }
                catch(...) { throw _STD_ runtime_error("Could not do subtraction for this matrix aa an error occured during memory allocation(which was required as the operator(-=) isn't noexcept for the template type T)."); }
                if constexpr ( noexcept( _STD_ declval<_STD_ decay_t<T>>() - _STD_ declval<_STD_ decay_t<T>>() ) && _STD_ is_nothrow_copy_constructible_v<T> ) {
                    #pragma omp parallel for collapse(2) schedule(static)
                    for (size_t i = 0; i < row; i++)
                        for (size_t j = 0; j < col; j++)
                    ...
                    for (size_t i = 0; i < row; i++) {
                        _TRY_CONSTRUCT_AT_LOOP_((j = 0), (j < col), (j++), result[i], m_data[i][j] - other.m_data[i][j])
                            ...
                            _MEM_ destroy_data<T>(m_data, i, j, row, col);
                            throw _STD_ runtime_error("Cannot subtract the two matrices because of error that occured in either copy construction of the matrix or the operator(-: binary) for the template type T failed(which was required because the template type doesn't have noexcept operator(-=), or failed (noexcept(T-T) && is_nothrow_copy_constructible_v))");
                        }
                    }
                }
                Matrix result_mat;
        ...
        requires compoundMultiplication<T> && compoundAddition<T> {
            ...
            T *to_transfer = _MEM_ allocate_memory<T*>(row);
            size_t d = 0;
            for (size_t i = 0; i < row; i++) {
                _MEM_ allocate_mem_2d_safe_continuous<T>(to_transfer, i, column);
            ...
            _STD_ swap(result.m_data, to_transfer); // m_data was nullptr before this.
            result.m_order = _MATRIX_ Order(row, column);
            if (result.is_wide()) {
                for (size_t i = 0; i < row; i++) {
    ...
    public:
        ...
        _MATRIX_ MatrixOneDIterator<T> begin_one_d() noexcept {
            return _MATRIX_ MatrixOneDIterator<T>(m_data, m_order.column());
        }
        _MATRIX_ MatrixOneDIterator<T> end_one_d() noexcept {
            return _MATRIX_ MatrixOneDIterator<T>(m_data, m_order.column(), m_order.size());
        }
        const _MATRIX_ MatrixOneDIterator<T> begin_one_d() const noexcept {
            return _MATRIX_ MatrixOneDIterator<T>(m_data, m_order.column());
        }
        const _MATRIX_ MatrixOneDIterator<T> end_one_d() const noexcept {
            return _MATRIX_ MatrixOneDIterator<T>(m_data, m_order.column(), m_order.size());
        }

        // Column one dimension iterators.
        _MATRIX_ MatrixOneDColumnIterator<T> begin_c_one_d() noexcept {
            return _MATRIX_ MatrixOneDColumnIterator<T>(m_data, m_order.column());
        }
        _MATRIX_ MatrixOneDColumnIterator<T> end_c_one_d() noexcept {
            return _MATRIX_ MatrixOneDColumnIterator<T>(m_data, m_order.column(), m_order.size());
        }
        const _MATRIX_ MatrixOneDColumnIterator<T> begin_c_one_d() const noexcept {
            return _MATRIX_ MatrixOneDColumnIterator<T>(m_data, m_order.column());
        }
        const _MATRIX_ MatrixOneDColumnIterator<T> end_c_one_d() const noexcept {
            return _MATRIX_ MatrixOneDColumnIterator<T>(m_data, m_order.column(), m_order.size());
        }

        // Iterators which provide row view object for each row.
        _MATRIX_ MatrixIterator<T> begin() noexcept {
            return _MATRIX_ MatrixIterator<T>(m_data, m_order.column());
        }
        _MATRIX_ MatrixIterator<T> end() noexcept {
            return _MATRIX_ MatrixIterator<T>(m_data + m_order.row(), m_order.column());
        }
        const _MATRIX_ MatrixIterator<T> begin() const noexcept {
            return _MATRIX_ MatrixIterator<T>(m_data, m_order.column());
        }
        const _MATRIX_ MatrixIterator<T> end() const noexcept {
            return _MATRIX_ MatrixIterator<T>(m_data + m_order.row(), m_order.column());
        }
    ...
    public:
        ...
        requires _CPY_CSTR_ {
            ...
            _STD_ swap(result.m_data, to_transfer); // Automatically sets to_transfer to nullptr.
            result.m_order = m_order.transpose();
            return result;
        }

        Matrix &transpose_in_place()
        requires (_CPY_CSTR_ || _STD_ is_nothrow_swappable_v<T>) {
            ...
            if constexpr (_STD_ is_nothrow_swappable_v<T>) {
                if (m_order.is_square()) {
                    ...
                    for (size_t i = 0; i < size; i++) {
                        ...
                        for (size_t j = i + 1; j < size; j++) _STD_ swap(data[j], m_data[j][i]);
                    }
                }
                else if constexpr (_CPY_CSTR_) *this = this->transpose();
                else throw _STD_ logic_error("Cannot do transposition on this Matrix because it is neither a square Matrix and is not copy constructible to create a new Matrix.");
            }
            else if constexpr (_CPY_CSTR_) *this = this->transpose();
            else throw _STD_ logic_error("Cannot do transposition on this Matrix because it is neither a square Matrix(or the type is not swappable) and is not copy constructible to create a new Matrix.");
            return *this;
        }
    ...
    public:
        ...
        requires compoundAddition<T> {
            ...
            _ZERO_EXISTS_
            if (m_order.is_zero()) {
                _NO_ZERO_COND_ throw _STD_ logic_error("Cannot provide the value of trace for a zero size Matrix with a type of which neither a default constructor exists nor is the zero value stored.");
            ...
            else throw _STD_ logic_error("Cannot provide the trace of the Matrix because it is not copy constructible for storing initial zero/default value and is neither default constructible.");
        }
    ...
    public:
        void shrink_columns_by(const size_t shrink_amount) noexcept {
            ...
            if (shrink_amount < col) {
                ...
                for (size_t i = 0; i < row; i++) _MEM_ reallocate_memory<T>(m_data[i], col, new_num_elements);
        ...
        requires _CPY_CSTR_ || _DFLT_CSTR_ {
            ...
            if constexpr (_CPY_CSTR_) {
                ...
                for (size_t i = 0; i < row; i++) {
                    ...
                    if constexpr (_STD_ is_nothrow_copy_constructible_v<T>) _STD_ uninitialized_fill_n(m_data[i] + col, extend_amount, zero);
        ...
        requires _CPY_CSTR_ {
            ...
            for (size_t i = 0; i < row; i++) {
                _MEM_ reallocate_memory(m_data[i], col, new_col);
                if constexpr (_STD_ is_nothrow_copy_constructible_v<T>) _STD_ uninitialized_fill_n(m_data[i] + col, extend_amount, copy_val);
    ...
    public:
        void shrink_rows_by(const size_t shrink_amount) noexcept {
            if (shrink_amount < m_order.row()) {
                _ROW_COL_
                for (size_t i = row - shrink_amount; i < row; i++) _MEM_ free_memory<T>(m_data[i], col);
                _MEM_ reallocate_memory<T*>(m_data, row, row - shrink_amount); // It is better to free the memory first because T* is trivially destructible and would not destroy the memory by itself.
        ...
        requires _CPY_CSTR_ || _DFLT_CSTR_ {
            ...
            for (size_t i = row; i < new_row; i++) {
                if constexpr (_CPY_CSTR_) {
                    ...
                    if constexpr (_STD_ is_nothrow_copy_constructible_v<T>) _STD_ uninitialized_fill_n(m_data[i], col, zero_val);
                    else _TRY_CONSTRUCT_AT_LOOP_(j, (j < col), (j++), m_data[i], zero_val) _CATCH_REW_RLR_(m_data, i, row, new_row, col, j)
                }
                else goto DEFAULT_CASE;
                continue;
                DEFAULT_CASE:
                    if constexpr (_STD_ is_nothrow_default_constructible_v<T>) _STD_ uninitialized_value_construct_n(m_data[i], col);
        ...
        requires _CPY_CSTR_ {
            ...
            _MEM_ reallocate_memory<T*>(m_data, row, new_row);
            for (size_t i = row; i < new_row; i++) {
                if constexpr (_STD_ is_nothrow_copy_constructible_v<T>) _STD_ uninitialized_fill_n(m_data[i], col, copy_val);
    ...
    public:
        ...
        requires _DFLT_CSTR_ || _CPY_CSTR_ {
            if (!m_order.is_zero()) {
            ...
            else *this = Matrix(_MATRIX_ Order(row_extend_amount, col_extend_amount));
        }
        ...
        requires _CPY_CSTR_ {
            ...
            else *this = Matrix(_MATRIX_ Order(row_extend_amount, col_extend_amount), copy_val);
        }
        ...
        requires _CPY_CSTR_ {
            ...
            else *this = Matrix(_MATRIX_ Order(row_extend_amount, col_extend_amount), row_extend_val);
        }
                ...
                if (row_extend_amount != 0) try {
                    ...
                    for (size_t i = row; i < new_row; i++) {
                        if constexpr (_STD_ is_nothrow_copy_constructible_v<T>) {
                            _STD_ uninitialized_fill_n(m_data[i], col, row_extend_val);
                            _STD_ uninitialized_fill_n(m_data[i] + col, col_extend_amount, common_extend_val);
                        }
                        else try {
                            for (j = 0; j < col; j++) _STD_ construct_at(m_data[i] + j, col_extend_val);
                            for (; j < new_col; j++) _STD_ construct_at(m_data[i] + j, common_extend_val);
